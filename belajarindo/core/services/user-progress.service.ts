...
export async function getProgressUsers({ query }: { query?: URLSearchParams | string }): Promise<ActionResponse<ActionGetListData<ProgressUsersDataTypes>>> {
  ...
  const payload = await transformResponse<GetData<ProgressUsersDataTypes>>(r.getWithFetch(), r.getRequestAndData())
...
export async function createProgressUser<T>({ data }: { data: T }): Promise<ActionResponse> {
...
export async function updateProgressUser<T>({ data }: { data: T }): Promise<ActionResponse> {
  const r = nextRequestChain(`${urls}`, {
    method: "PUT",
    body: JSON.stringify(data)
  }).withAuth()
...
export async function deleteProgressUser({ id }: { id: string }): Promise<ActionResponse> {
  const r = nextRequestChain(`${urls}`, {
    method: "DELETE",
